...
class User::Token::Creation < Solid::Process
  ...
  def validate_token_existence(user:, **)
    ...
    token&.persisted? ? Success(:token_already_exists, token: token) : Continue()
  end

  def create_token_if_not_exists(user:, executed_at:, **)
    ::RuntimeBreaker.try_to_interrupt("USER_TOKEN_CREATION")
    ...
    token.persisted? ? Continue(token: token) : Failure(:token_creation_failed, **token.errors.messages)
  end
end
